## EH-Translator 0.0.1
- 完全移除个人配置、规则与生词本的 Worker、WebDAV、GitHub Gist 云同步运行时及 WebDAV 依赖；订阅规则远程拉取与每日更新仍予保留。
- 简化基本设置页面：仅保留高频选项，低频选项折叠到高级设置中；移除日志级别 UI，保留运行时配置以兼容已有设置。
- 简化接口设置详情：主界面保留首次配置与日常使用字段，其余调参收纳到可折叠的高级设置中；保留运行时配置字段和请求行为不变。
- 新安装默认仅保留 DeepSeek 与自定义接口；其他内置翻译服务仍可在设置页按需添加。
- 网页、划词与字幕翻译默认使用 DeepSeek；填写基础地址时会自动补全 OpenAI 兼容的 `/chat/completions` 路径。
- 删除选项页的同步设置模块、导航入口及其专用配置文案。
- 简化划词、鼠标悬停与字幕设置页面：主界面保留日常常用项，其余低频参数折叠到高级设置中。
